...
class GUIActions(QObject):
    ...
    def __init__(self, gui_app: Ui_MainWindow, gui_updater: GUIUpdater):
        ...
        self.start_end_lap_button_mode = ButtonModes.START
    ...
    def start_stop_button_clicked(self) -> None:
        if self.start_end_lap_button_mode is ButtonModes.START:  # If the mode is Start
            ...
            self.start_end_lap_button_mode = ButtonModes.STOP
            self.gui_app.start_end_button.setText("End Lap")
            # Emit a start signal
            self.signal_start.emit(connection_type, lap_name)
        else:  # If the mode is stop
            # Set the mode and button text to start
            self.start_end_lap_button_mode = ButtonModes.START
            self.gui_app.start_end_button.setText("Start Lap")
            # Emit a stop signal
